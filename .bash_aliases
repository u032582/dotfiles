...
alias awsmfa='export -n AWS_ACCESS_KEY_ID && export -n AWS_SECRET_ACCESS_KEY && export -n AWS_SESSION_TOKEN && echo -n "enter token: " && read token && source <(aws sts get-session-token --token-code "$token" --serial-number ${AWS_MFA_ARN} --output json | jq -r ".Credentials | [\"export AWS_ACCESS_KEY_ID=\(.AccessKeyId)\", \"export AWS_SECRET_ACCESS_KEY=\(.SecretAccessKey)\",\"export AWS_SESSION_TOKEN=\(.SessionToken)\", \"echo Expiration: \(.Expiration)\"] | @tsv" | tr "\t" "\n")'
